...
class AgentService:
    ...
    def _classify_command(self, text: str) -> str:
        ...
        if any(keyword in text_lower for keyword in ["schedule", "create event", "add event", "new event"]):
            return "create_event"
        
        # Create task patterns
        if any(keyword in text_lower for keyword in ["task", "todo", "to do", "to-do", "add task"]):
            return "create_task"
        
        # Show events patterns
        if any(keyword in text_lower for keyword in ["show events", "list events", "display events", "what events", "my events"]):
            return "show_events"
        
        # Show tasks patterns
        if any(keyword in text_lower for keyword in ["show tasks", "list tasks", "display tasks", "what tasks", "my tasks"]):
            return "show_tasks"
        
        # Update task status patterns
        if any(keyword in text_lower for keyword in ["mark", "complete", "finish", "done with"]) and "task" in text_lower:
            return "update_task_status"
        
        # Default to creating an event if it contains time expressions
        time_indicators = ["today", "tomorrow", "next week", "monday", "tuesday", "wednesday", 
    ...
    def _handle_show_events(self, user_id: int, text: str) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "events": event_list,
                "date_range": {
                    "start": date_range[0].strftime("%Y-%m-%d") if date_range[0] else "all past events",
                    "end": date_range[1].strftime("%Y-%m-%d") if date_range[1] else "all future events"
                }
            }
        except Exception as e:
    ...
    def _extract_date_range(self, text: str) -> Tuple[Optional[datetime], Optional[datetime]]:
